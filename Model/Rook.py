...
class Rook(Piece):
    def get_valid_moves(self, board):
        """
        Trả về danh sách nước đi hợp lệ của quân Rook.
        ...
        # Di chuyển xuống (Player 1) hoặc lên (Player 2)
        new_y = self.y + direction
        if 0 <= new_y < 9 and board[new_y][self.x] is None:
            moves.append((self.x, new_y))

        # Di chuyển ngang (trái/phải, không giới hạn)
        for dx in [-1, 1]:
            new_x = self.x + dx
